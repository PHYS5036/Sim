		...
		const char* partname = "all" )
{
  ...
  TLatex* tex = new TLatex( 0.2, 0.9, Form("Mean voxel energy deposit per incident particle = %f MeV",(edep0tot+edep1tot)/(float)nentries/(float)nbins_nz));
  tex->SetNDC(1);
  tex->SetTextFont(42);
  tex->SetTextColor(2);
