...
// It plots the energy deposited as a function of transverse distance
// in the phantom for healthy tissue and the tumour.
//
// P423M Lab - David Hamilton
//
// root -b -q PlotDoseT.C++
//
// -----------------------------------------------------------------------------
...
void PlotDoseT( const char* finname  = "~/data/G4_BEAMX_ENERGYY.root", 
		const char* partname = "all" )
{
  ...
  float tmin  = -50;
  float tmax  = 50;
  Int_t nbins = (tmax - tmin)/voxelsize;    
  float edep0tot = 0.0;
  float edep1tot = 0.0;
  float tpos[nbins], edep0[nbins], edep1[nbins];

  for ( Int_t k = 0; k < nbins ; k++ ) {	      
    tpos[k]  = (tmin +  k * voxelsize);
    edep0[k] = 0.0;
    edep1[k] = 0.0;
  }
  ...
  for ( Long64_t i = 0; i < nentries; i++ ) {
	...
	if( Prim_Th > 1.57 && (Prim_Ph < -1.57 || Prim_Ph > 1.57) ) {  // top or bottom
	  if( Phantom_x[j] > lmin && Phantom_x[j] < lmax && Phantom_y[j] > lmin && Phantom_y[j] < lmax ) { 
	    if( Phantom_z[j] > (tmin + k*voxelsize) && Phantom_z[j] < (tmin + (k+1)*voxelsize) ) {
	...
	if( Prim_Ph == 0 && (Prim_Th < 1.57 || Prim_Th > 3.1) ) { // front or back
	  if( Phantom_x[j] > lmin && Phantom_x[j] < lmax && Phantom_z[j] > lmin && Phantom_z[j] < lmax ) { 
	    if( Phantom_y[j] > (tmin + k*voxelsize) && Phantom_y[j] < (tmin + (k+1)*voxelsize) ) {
  ...
  TGraph *g, *g1;
  if( tum_max > nor_max ) {
    g = new TGraph(nbins, tpos, edep1 );
    g->SetMarkerColor( 2 );
    g->SetLineColor( 2 );
  }
  else {
    g = new TGraph(nbins, tpos, edep0 );
    g->SetMarkerColor( 4 );
    g->SetLineColor( 4 );
  }
  ...
  g->GetXaxis()->SetTitle("Transverse Distance (mm)");
  g->GetYaxis()->SetTitle("Energy Deposited (MeV)");

  if( tum_max > nor_max ) {
    g1 = new TGraph(nbins, tpos, edep0 );
    g1->SetMarkerColor( 4 );
    g1->SetLineColor( 4 );
  }
  else {
    g1 = new TGraph(nbins, tpos, edep1 );
    g1->SetMarkerColor( 2 );
    g1->SetLineColor( 2 );
  }
